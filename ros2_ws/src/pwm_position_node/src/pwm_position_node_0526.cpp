...
#include "idc_node/msg/joint_plot_data.hpp" // <--- ADDED: Include for plotting message

using namespace std::chrono_literals;
using SetXY = inverse_kinematics_node::msg::SetXY;
using JointPlotData = idc_node::msg::JointPlotData; // <--- ADDED: Alias for convenience
...
public:
  PWMNode():Node("pwm_position_node")
  {
    ...
    // --- ADDED: Initialize the plot data publisher ---
    plot_data_pub_ = create_publisher<JointPlotData>("pwm_joint_plot_data", 10);
    // --- END ADDED ---
...
/* ---- control loop --------------------------------------------------- */
  void loop()
  {
    ...
    // --- ADDED: Publish plot data ---
    auto plot_msg = JointPlotData();
    plot_msg.measured_q1 = cur1; // cur1 is in degrees
    plot_msg.desired_q1 = g1;    // g1 is in degrees
    plot_msg.measured_q2 = cur2; // cur2 is in degrees
    plot_msg.desired_q2 = g2;    // g2 is in degrees
...
/* ---- data members --------------------------------------------------- */
  ...
  rclcpp::Publisher<JointPlotData>::SharedPtr plot_data_pub_; // <--- ADDED: Publisher member variable
  std::mutex mut_;

...
class motorControlAn:
    ...
    def __init__(
        ...
        # Set the position set-point in units of degrees
        self.position_set_point_deg = max(
            min(ang_set_point_deg, motorE.model_info.max_angle_deg), 0.0
        )
        ...
        # Put motor in "home" posiiton
        self.motorE.set_mode(DynamixelMode.Position)
        self.motorS.set_mode(DynamixelMode.Position)
        self.motorE.enable_torque()
        self.motorS.enable_torque()
        self.motorE.angle_deg = 0.0
        self.motorS.angle_deg = 0.0
        while abs(self.motorE.angle_deg) > 0.8 or abs(self.motorS.angle_deg) > 0.8:
            self.motorE.angle_deg = 0.0
            self.motorS.angle_deg = 0.0
            time.sleep(0.5)

        self.motorE.disable_torque()
        self.motorS.disable_torque()
        time.sleep(0.5)

        # PWM Mode (i.e. voltage control)
        self.pwm_limit = float(self.motorE.motor_info.pwm_limit)
        self.pwm_limit = float(self.motorS.motor_info.pwm_limit)
        # motorE.set_mode(DynamixelMode.PWM)
        # motorS.set_mode(DynamixelMode.PWM)

    # Defining position values to be stored
    @property
    def position_history(self) -> NDArray[np.double]:
        return np.asarray(self._position_history)
    ...
    def start(self):
        ...
        curr_time = time.time_ns()
        # while self.should_continue:
        #     # angE_deg = self.motorE.angle_deg
        #     angS_deg = self.motorS.angle_deg

        #     self._position_history.append(angS_deg)  # Save for plotting
        #     self._timestamps.append(time.time())  # Save for plotting
        ...
        self.motorE.angle_deg = 90.0
        self.motorS.angle_deg = 90.0
        time.sleep(0.5)
...
if __name__ == "__main__":
    ...
    motorE = motor_factory.create(4)
    motorS = motor_factory.create(2)

    # Make controller
    # TODO: Replace all "..." below with your selected choices of gains.
    controller = motorControlAn(
        motorE=motorE,
        motorS=motorS,
        proportional_gain=1,
        integral_gain=0.001,
